...
class Classifier(object):
    ...
    def classify(self, images: tf.Tensor, evaluation_mode: bool=False) -> tf.Tensor:
        """
        Parameters
        ----------
        images
        ...
        """
        with tf.variable_scope('feed_forward', reuse=evaluation_mode):
            ...
            fully_connected_2 = self._fully_connected_layer(fully_connected_1, outputs_number=192,
                                                            name='fully_connected_2', evaluation_mode=evaluation_mode)
            linear = self._linear(fully_connected_2, evaluation_mode=evaluation_mode)

        return linear

    def _conv(self, input_tensor, filter_edge_length, num_of_output_channels, name, evaluation_mode):
        with tf.variable_scope(name, reuse=evaluation_mode) as scope:
    ...
    def _fully_connected_layer(self, input_tensor, outputs_number, name, evaluation_mode):
        with tf.variable_scope(name, reuse=evaluation_mode) as scope:
    ...
    def _linear(self, input_tensor, evaluation_mode):
        with tf.variable_scope('linear', reuse=evaluation_mode) as scope:
            inputs_length = input_tensor.get_shape()[-1].value
            weights = self._create_variable('weights', [inputs_length, DataLoader.NUM_CLASSES],
                                            stddev=1/inputs_length, weight_decay=0.0, evaluation_mode=evaluation_mode)
    ...
    def _create_variable(self, name, shape, stddev, weight_decay, evaluation_mode):
        """
        ...
        # don't add loss to losses collection if the net is run in evaluation mode (instead of training mode)
        if weight_decay is not None and not evaluation_mode:
